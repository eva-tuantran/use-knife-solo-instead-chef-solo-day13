...
function the_bootstrap_widgets_init() {
	...
	register_sidebar( array(
		...
		'after_widget'	=>	'</aside>',
		'before_title'	=>	'<h2 class="subtitle widget-title">',
		'after_title'	=>	'</h2>',
	) );
...
function the_bootstrap_continue_reading_link() {
	//return ' <a href="'. esc_url( get_permalink() ) . '">' . __( '続きを読む <span class="meta-nav">&rarr;</span>', 'the-bootstrap' ) . '</a>';
	// 全体リンク化により<a>タグを外す
	return '続きを読む &rarr;';
}
...
// header内に条件によってタグを追加する
function head_additional_load(){
    if(is_single() || is_page()){
...
add_action('wp_head', 'head_additional_load');

function set_special_base_url() {
	global $special_base_url;
	/*
	if(isset($_SERVER['HTTP_X_FORWARDED_HOST']) && $_SERVER['HTTP_X_FORWARDED_HOST'] == 'aucfan.com'){
		$special_base_url = 'aucfan.com/article';
	}else{
		$special_base_url = 'special.aucfan.com';
	}
	*/
	$special_base_url = 'www.rakuichi-rakuza.jp/blog';
}
add_action( 'after_setup_theme', 'set_special_base_url' );

function get_special_base_url(){
	global $special_base_url;
	return $special_base_url;
}
function replace_special_link($link){
	global $special_base_url;
	if( false === strpos($link, $special_base_url) ){
		$link = str_replace('special.aucfan.com', $special_base_url, $link);
	}
	return $link;
}
add_action( 'bcn_breadcrumb_url', 'replace_special_link' );
