...
var baseUrl = "/special/index/load";

function bodyInit(){
	loadChartDataList();
}

function loadChartDataList(){
	$.ajax({
		type: 'post',
		dataType:'json',
		url: baseUrl,
		data: request,
		success: loadChartDataListSuccess
	});
}

function loadChartDataListSuccess(data,status){
	if(status != "success"){
		alert("システムエラー。");
		isServiceError = true;
		return;
	...
	$("#loading_chart_data").remove();
	chartDataList = data;

	iphoneChart.show('spec0');	

	ShowAllChart();
}

function ShowAllChart(){
	jQuery.each(ShowChart, function(key, val) {
		eval("ShowChart." + key + "();");
	});
}
...
var iphoneChart = {
	...
	change : function(spec){
		var chartVal = chartDataList.chartData;
		var lineData = chartVal.spec0;
		var model = chartDataList.model;
		console.log(chartDataList.dateCategories);
		var options = {renderTo : "iphoneCharts", title : "", unit : "円", lineData : lineData, dateCategorys : chartDataList.dateCategories};
		this.create(options);
		var items = this.getCompanyColorItems();
		var defaultCompanyColor;
	...
	showPrice : function(companyColor, objectId){
		...
		$(".price .bid a.desktop_hidden").attr("href", avgPriceUrl_ip);
	},
	setStatus : function(memoryFlg){
		var itemNames = request['model']['name'];
	...
	create : function(options){
		var chart = new Highcharts.Chart({
			...
			xAxis: {
                categories: options.dateCategories
            },
			yAxis : {
				title : {
					text : ""
