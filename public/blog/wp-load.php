...
// wp-config.phpを環境によって呼び出し分ける
if (isset($_SERVER['FUEL_ENV'])) {
	$env = $_SERVER['FUEL_ENV'] . '.';
} else {
	$env = '';
}
...
} elseif ( file_exists( dirname(ABSPATH) . '/wp-config.'.$env.'php' ) && ! file_exists( dirname(ABSPATH) . '/wp-settings.php' ) ) {

	/** The config file resides one level above ABSPATH but is not part of another install */
	require_once( dirname(ABSPATH) . '/wp-config.'.$env.'php' );
